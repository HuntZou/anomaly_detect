...
class OEImageNet(torchvision.datasets.ImageNet):
    def __init__(self, size: torch.Size, root: str = None, split='val', limit_var: int = np.infty, exclude: List[str] = ()):
        """
        Outlier Exposure dataset for ImageNet. ImageNet的离群曝光数据集。
        :param size: size of the samples in n x c x h x w, samples will be resized to h x w. If n is larger than the
            number of samples available in ImageNet, dataset will be enlarged by repetitions to fit n.如果n大于ImageNet中可用的样本数量，数据集将通过重复放大以适应n。
            This is important as exactly n images are extracted per iteration of the data_loader. 这一点很重要，因为data_loader每次迭代都精确地提取n个图像。
            For online supervision n should be set to 1 because only one sample is extracted at a time. 对于在线监督，n应该设置为1，因为一次只能提取一个样本。
        :param root: root directory where data is found or is to be downloaded to. 找到或下载数据的根目录。
        ...
        :param limit_var: limits the number of different samples, i.e. randomly chooses limit_var many samples.
            ...
            限制不同样本的数量，即随机选择limit_var多个样本。
            从所有可用的数据中提取训练数据。
        :param exclude: all class names that are to be excluded.要排除的所有类名。
        """
        ...
        if exclude is not None and len(exclude) > 0:
            syns = {k: v.lower().replace(' ', ',').split(',') for k, v in IMAGENET1k_CLS_STR.items()}  # 将字符串中的大写字母换成小写字母 split按','划分列表
            exclude_ids = [i for i, s in syns.items() if any([exs.lower() in s for exs in exclude])]  # 打印出要排除的类名
            self.picks = np.argwhere(np.isin(self.targets, exclude_ids, invert=True)).flatten().tolist()  # 返回不是exclude_ids类的索引值
            # self.show()
            # print()
    ...
    def __getitem__(self, index):
        ...
        sample = sample.mul(255).byte()  # 进行空间转换
