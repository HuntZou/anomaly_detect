...
def get_target_label_idx(labels: np.ndarray, targets: np.ndarray):
    """
    Get the indices of labels that are included in targets. 获取目标中包含的标签的索引
    :param labels: array of labels
    :param targets: list/tuple of target labels 目标标签的列表/元组
    :return: list with indices of target labels 带有目标标签索引的列表
    """
...
def local_contrast_normalization(x: torch.tensor, scale: str = 'l2'):
    ...
    对张量应用局部对比度归一化，即减去特征（像素）的平均值并按比例归一化
    这是跨越特征（像素）的标准偏差、L1或L2范数
    请注意这是针对所有像素（而不是数据集）的‘每样本’标准化
    """

    assert scale in ('l1', 'l2')

    n_features = int(np.prod(x.shape))  # 特征个数

    mean = torch.mean(x)  # mean over all features (pixels) per sample
    x -= mean

    if scale == 'l1':
        x_scale = torch.mean(torch.abs(x))  # torch.abs(x)绝对值
...
class MultiCompose(transforms.Compose):
    ...
    和transforms.compose类似，但将转换应用于多个变量，而不是一个变量
    最重要的是，对于随即变换（如RandomCrop），应用相同变换选择 即例如所有变量都是相同的作物
    """
